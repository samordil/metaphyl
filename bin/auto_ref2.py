...
def process_reference(args: Tuple[str, str, Path, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Clean up temp files but keep output files
        for f in temp_files:
            try:
                if os.path.exists(f):
                    os.remove(f)
    ...
    try:
        ...
        try:
            ...
            metadata = {
                ...
                'output_files': {
                    'sorted_bam': best_ref.get('output_files', {}).get('sorted_bam'),
                    'bai_file': best_ref.get('output_files', {}).get('bai_file')
                }
            }
            ...
            # Move BAM and BAI files to final location
            if 'output_files' in best_ref:
                final_bam = output.with_suffix('.sorted.bam')
                final_bai = output.with_suffix('.sorted.bam.bai')
                
                os.rename(best_ref['output_files']['sorted_bam'], final_bam)
                os.rename(best_ref['output_files']['bai_file'], final_bai)
                
                logging.info(f"Saved sorted BAM file: {final_bam}")
                logging.info(f"Saved BAM index file: {final_bai}")
